...
def _apply_database_migrations():
    ...
    with engine.begin() as conn:
        ...
        existing = set(inspector.get_table_names(schema="social"))
        required = {"social_connections", "publish_jobs", "post_media", "processed_events"}
        if not required.issubset(existing):
            conn.execute(text("CREATE SCHEMA IF NOT EXISTS social"))
            conn.execute(text("""
                DO $$ BEGIN
                    CREATE TYPE social.publish_status AS ENUM ('pending', 'publishing', 'published', 'failed');
                EXCEPTION WHEN duplicate_object THEN null;
                END $$;
            """))
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS social.social_connections (
                    account_id VARCHAR PRIMARY KEY,
                    linkedin_member_urn VARCHAR NOT NULL,
            ...
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS social.publish_jobs (
                    ...
                    status social.publish_status NOT NULL,
            ...
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS social.post_media (
                    id VARCHAR PRIMARY KEY,
                    publish_job_id VARCHAR NOT NULL REFERENCES social.publish_jobs(id),
                    linkedin_asset_urn VARCHAR NOT NULL,
                    image_url VARCHAR NOT NULL
                )
            """))
            conn.execute(text("""
                CREATE TABLE IF NOT EXISTS social.processed_events (
                    event_id VARCHAR PRIMARY KEY,
                    processed_at TIMESTAMPTZ DEFAULT now()
                )
            """))
            conn.execute(text("CREATE INDEX IF NOT EXISTS ix_social_publish_jobs_scheduled_post_id ON social.publish_jobs (scheduled_post_id)"))
    yield
...
def _patch_token_encryption_key(monkeypatch):
    monkeypatch.setattr("app.config.settings.token_encryption_key", Fernet.generate_key().decode())
