...
def require_owner(payload: dict) -> None:
    """Trusts the JWT's role claim directly rather than a live cross-
    service call to user-service — deliberate simplification given time
    constraints, not the get_live_membership pattern used elsewhere.
    Acceptable staleness window: access tokens expire in 15 minutes, and
    the gateway already requires an account-scoped token to reach this
    service at all (billing isn't on account_scope_exempt_routes)."""
    if payload.get("role") != "owner":
        raise _forbidden("insufficient_role", "This action requires the owner role.")